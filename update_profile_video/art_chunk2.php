...
if (!$chunks || $chunk == $chunks - 1) {
    ...
    $count = $stmt->rowCount();
    if ($count > 0) {
        $stmt = $connect->prepare("UPDATE art_register SET video2=?  WHERE username=?");
        $stmt->execute(array(
            $fileName,
            $_SESSION['username'],
        ));
    ...
    $count = $stmt->rowCount();
    if ($count > 0) {
        $stmt = $connect->prepare("UPDATE fash_register SET video2=?  WHERE username=?");
        $stmt->execute(array(
            $fileName,
            $_SESSION['username'],
        ));
    ...
    $count = $stmt->rowCount();
    if ($count > 0) {
        $stmt = $connect->prepare("UPDATE sport_register SET video2=?  WHERE username=?");
        $stmt->execute(array(
            $fileName,
            $_SESSION['username'],
        ));
